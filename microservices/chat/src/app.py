...
# Using the verified secret from your environment
...
def token_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        try:
            ...
            if not current_user_id: return jsonify({'error': 'Invalid token'}), 401
...
def get_db_connection():
    try:
        conn = psycopg2.connect(host=DB_HOST, port=DB_PORT, database=DB_NAME, user=DB_USER, password=DB_PASSWORD, cursor_factory=RealDictCursor)
        return conn
    except Exception as e:
...
def generate_ai_response(message, conversation_history=None, document_context=None):
    if not openrouter_client:
        return f"I understand you said: '{message}'. How can I help you?"
    try:
        ...
        # Updated to an active free model
        payload = {
...
def publish_to_kafka(message):
    if not kafka_producer: return False
    try:
...
def send_message_simple(current_user_id):
    try:
        ...
        cursor = conn.cursor()
        # Get history
        cursor.execute("SELECT role, content FROM chat_messages WHERE session_id = %s ORDER BY created_at ASC", (conversation_id,))
        history = [dict(msg) for msg in cursor.fetchall()]
        
        ai_response = generate_ai_response(message, history)
        
        # Store messages
        cursor.execute("INSERT INTO chat_messages (message_id, session_id, user_id, role, content) VALUES (%s, %s, %s, %s, %s)",
                       (str(uuid.uuid4()), conversation_id, current_user_id, 'user', message))
        cursor.execute("INSERT INTO chat_messages (message_id, session_id, user_id, role, content) VALUES (%s, %s, %s, %s, %s)",
                       (str(uuid.uuid4()), conversation_id, current_user_id, 'assistant', ai_response))
        ...
        return jsonify({'conversation_id': conversation_id, 'ai_response': {'content': ai_response}}), 200
    except Exception as e:
